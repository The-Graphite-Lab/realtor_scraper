...
class RealtorScraper(Scraper):
    ...
    def handle_location(self):
        ...
        if not result:
            raise NoResultsFound(
                "No results found for location: " + self.location)
    ...
    def handle_listing(self, listing_id: str) -> list[Property]:
        ...
        listing = Property(
            ...
            latitude=property_info["address"]["location"]["coordinate"].get(
                "lat")
            if able_to_get_lat_long
            else None,
            longitude=property_info["address"]["location"]["coordinate"].get(
                "lon")
            if able_to_get_lat_long
            else None,
            address=self._parse_address(
                property_info, search_type="handle_listing"),
    ...
    def get_latest_listing_id(self, property_id: str) -> str | None:
        ...
        primary_listing = next(
            (listing for listing in property_info["listings"]
             if listing["primary"]),
            None,
        )
        if primary_listing:
    ...
    def general_search(
        ...
        for result in response_json["data"][search_key]["results"]:
            ...
            is_pending = result["flags"].get(
                "is_pending") or result["flags"].get("is_contingent")
            ...
            realty_property = Property(
                ...
                longitude=result["location"]["address"]["coordinate"].get(
                    "lon")
                if able_to_get_lat_long
                else None,
                address=self._parse_address(
                    result, search_type="general_search"),
